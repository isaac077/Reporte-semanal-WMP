...
function setupApiRoutes(app2) {
  ...
  app2.get("/api/report/pdf", async (req, res) => {
    try {
      ...
      const base64Pdf = await page.evaluate(async () => {
        for (let i = 0; i < 150; i++) {
          if (window.generatePdfAsBase64) {
            return await window.generatePdfAsBase64();
          }
  ...
  const handleRpcRequest = async (jsonRpcReq, req) => {
...
${JSON.stringify(updated, null, 2)}`
  ...
  const handleMcpMessage = async (req, res) => {
    ...
    const body = req.body;
    const response = Array.isArray(body) ? await Promise.all(body.map((b) => handleRpcRequest(b, req))) : await handleRpcRequest(body, req);
    if (response) {
